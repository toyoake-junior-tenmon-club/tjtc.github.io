...
class ObjectGroup {
    ...
    static createObject = (title, objectGeometry) => {
        let surface_color = { color: 0xffff00 } ;
        switch (title) {
            case OBJECTS.SUN:
                break;
            case OBJECTS.EARTH:
                surface_color = { color: 0x8888ff } ;
                break;
            default:
                surface_color = { color: 0x888888 } ;
                break;
        }
        const objectMaterial = new THREE.MeshBasicMaterial( surface_color );
